...
GitHub の Dependabot により、毎週月曜早朝に以下の自動チェックが行われます：

- npm / pnpm 依存（package.json + pnpm-lock.yaml）
- .github/workflows で使用している GitHub Actions

更新が見つかった場合、関連するパッケージをグループ化して PR が作成されます（例: React 系、TanStack 系、Three.js 系、Storybook 系、テスト系）。

PR には `dependencies` および `area:deps`（または `ci`）ラベルが付き、通常の CI（E2E 含む）が実行されます。`main` ブランチの ruleset により PR と必須チェック通過が必要です。

#### 日常業務フローへの影響

- **機能開発のフロー自体に変更はありません**。feature/\* ブランチでの開発、PR 作成、レビューはこれまで通りです。
- 新たに発生するのは「メンテナンス PR のレビュー作業」です。月曜早朝に Dependabot PR が数件（グループ化されているため多くても数個程度）作成されます。
- これらの PR は通常の PR レビューと同じプロセスで対応してください（CI 通過確認 + 必要に応じたレビュー）。
- 自動マージは一切行われません（`main` ruleset により PR と必須チェック通過が必要）。
- Major アップデート（例: React 18→19, Vite 6→8 など）が含まれる場合は breaking change の可能性があるので、CHANGELOG や影響範囲を特に確認してください。必要に応じてローカルで `pnpm install` して手動テストすることを推奨します。
- 必要に応じて後で `ignore` や `groups` の調整、または特定のバージョンの手動対応も可能です。

Dependabot PR をレビューする際は、以下の点に特に注意してください：

- CI がすべて通っていることを確認（特に E2E と a11y）
- 更新内容にセキュリティ修正が含まれていないか確認
- 破壊的変更がある場合はドキュメントやサンプルコードへの影響をチェック

この仕組みにより、手動で `pnpm outdated` を定期的にチェックする手間が減り、依存の陳腐化を防ぎやすくなります。
...
`main` ブランチには GitHub Rulesets の `Protect main` により、以下の保護ルールが適用されています:

- `main` への直接プッシュ禁止（必ず PR 経由）
- Force push・ブランチ削除の禁止
- 必須ステータスチェック `ci` と `e2e` の成功が必要
- レビュー承認と CODEOWNERS 承認は現時点では必須ではありません
