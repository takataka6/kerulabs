...
PR には `dependencies` および `area:deps`（または `ci`）ラベルが付き、通常の CI（E2E 含む）が実行されます。ブランチ保護によりレビューと全チェック通過が必要です。
...
- これらの PR は通常の PR レビューと同じプロセスで対応してください（CI 通過確認 + レビュー承認）。
- 自動マージは一切行われません（ブランチ保護ルールにより必須レビュー + 全 CI 成功が必要）。
