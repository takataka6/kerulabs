...
describe("useTacticsOrchestration — 派生状態", () => {
  ...
  describe("tacticsForCurrentFormation", () => {
    ...
    it("selectedTeam に whitelist がある場合それでさらにフィルタする", () => {
      const customTactic = createTestTactic({
        id: "custom-1",
        formationName: "4-3-3",
      });
      const defaultTactic = createTestTactic({
        id: "default-1",
        isCustom: false,
        formationName: "4-3-3",
      });
      const excludedDefault = createTestTactic({
        id: "default-2",
        isCustom: false,
        formationName: "4-3-3",
      });
      ...
      const mockTeam = {
        getAvailableTacticsForFormation: vi.fn().mockReturnValue(["default-1"]),
      } as unknown as Parameters<
        typeof useTacticsOrchestration
      >[0]["selectedTeam"];

      const params = createDefaultParams({
        tactics: [customTactic, defaultTactic, excludedDefault],
      ...
      // customTactic は isCustom なので whitelist 関係なく含まれる
      // defaultTactic は whitelist に含まれるので含まれる
      // excludedDefault は whitelist に含まれないので除外
      expect(result.current.tacticsForCurrentFormation).toEqual([
        customTactic,
        defaultTactic,
      ]);
    ...
    it("selectedTeam の whitelist が undefined (制限なし) の場合全戦術を返す", () => {
      const t1 = createTestTactic({
        id: "t-1",
        formationName: "4-3-3",
        isCustom: false,
      });
      const t2 = createTestTactic({ id: "t-2", formationName: "4-3-3" });

      const mockTeam = {
        getAvailableTacticsForFormation: vi.fn().mockReturnValue(undefined),
      } as unknown as Parameters<
        typeof useTacticsOrchestration
      >[0]["selectedTeam"];
      ...
      expect(result.current.tacticsForCurrentFormation).toEqual([t1, t2]);
    });
  });
