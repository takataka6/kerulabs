...
- **相手フォーメーションの全パターン選択に対応**: 戦術ビューアーの相手チーム設定で、相手側のフォーメーションを制限なく選べるようにしました。

### Changed

- **スケッチモード時のサイドバー挙動を調整**: 戦術ビューアー内の右サイドバーと関連UIの振る舞いを見直し、スケッチ操作中でも意図どおり扱いやすい状態に整えました。

### Fixed

- **戦術ビューアーのモード競合を修正**: スケッチや閲覧系のモード切り替えが衝突してUI状態が不整合になるケースを解消しました。
